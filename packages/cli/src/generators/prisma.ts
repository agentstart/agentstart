...
FEATURES:
  - Inspects existing schema and merges new model definitions
  - Handles many-to-many relationships via AST transforms
  - Seeds generator when schema file is missing
SEARCHABLE: prisma schema, cli generator, database scaffolding
agent-frontmatter:end */

import path from "node:path";
import { type FieldType, getTables } from "@agent-stack/infra/db";
...
export const generatePrismaSchema: SchemaGenerator = async ({
  ...
  const prismaOptions = (adapter.options ?? {}) as Partial<{
    provider: "postgresql" | "mysql" | "sqlite" | "mongodb";
  }>;
  const provider = prismaOptions.provider ?? "postgresql";
  const tables = getTables(options);
  const filePath = file || "./prisma/schema.prisma";
  const schemaPrismaExist = fs.existsSync(path.join(process.cwd(), filePath));
  let schemaPrisma = "";
    ...
    for (const field in fields) {
      ...
      if (attr.references) {
        ...
        if (!manyToManyRelations.has(referencedModel)) {
          manyToManyRelations.set(referencedModel, new Set<string>());
        }
        const relations = manyToManyRelations.get(referencedModel)!;
        relations.add(capitalizeFirstLetter(table));
      }
    }
  }
