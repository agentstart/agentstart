...
PURPOSE: Implements Grep tool execution within the AgentStart runtime.
USAGE: Register the "grep" tool when composing the agent configuration to expose this capability.
EXPORTS: grep
FEATURES:
  - Bridges sandbox APIs into the Grep workflow
  - Streams structured progress updates and normalizes tool output
SEARCHABLE: packages, agentstart, src, agent, tools, grep, index, tool, runtime
  ...
  async *execute(
    ...
    if (!sandbox) {
      yield {
        ...
        error: {
          message:
            "Grep tool requires a sandbox to be configured. Please configure a sandbox adapter in your AgentStart options.",
        },
      } satisfies AgentStartToolOutput["grep"];
      return;
    }
