...
USAGE: import { getAdapter } from "agentstart/db/utils"
...
import { kyselyAdapter } from "@/db/adapter/kysely";
import { createKyselyAdapter } from "@/db/adapter/kysely/dialect";
import { memoryAdapter } from "@/db/adapter/memory";
import type { Adapter, AgentStartOptions } from "@/types";
import { type FieldAttribute, getTables } from ".";

export async function getAdapter(options: AgentStartOptions): Promise<Adapter> {
  if (!options.memory) {
