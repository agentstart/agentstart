...
export type Thread = Awaited<ReturnType<typeof start.api.thread.get>>["thread"];
export type UIMessage = Awaited<ReturnType<typeof start.api.message.get>>[number];
```
...
//    ^? (params: { text?: string; files?: BlobFileList }) => Promise<void>
```
...
//    ^? { id: string; title: string; visibility: "public" | "private"; createdAt: Date; ... }

// Message type is inferred
const messages = await start.api.message.get({ threadId: thread.id });
//    ^? Array<AgentStartUIMessage>
```
...
```ts
function isTextMessage(message: UIMessage): message is UIMessage & {
  parts: [{ type: "text"; text: string }, ...UIMessage["parts"]];
} {
  return message.parts?.some((part) => part.type === "text") ?? false;
}

const messages = await start.api.message.get({ threadId });
