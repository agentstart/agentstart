...
```ts
const models = await start.api.model.list();
```
...
<TypeTable
  type={{
    models: { type: 'Array<object>', typeDescription: 'Array<{ id: string; name: string; provider: string }>', required: true },
    defaultModelId: { type: 'string | null', description: 'Accepts null.', required: true }
  }}
/>
