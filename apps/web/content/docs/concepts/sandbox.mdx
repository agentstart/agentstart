...
<Callout>
  Sandbox is optional. If you skip configuration, sandbox routes and OS tools stay disabled and any OS tool call will return “Sandbox not configured”. For production deployments with code isolation requirements, explicitly configure a sandbox (Node.js or E2B).
</Callout>

## Enabling Sandbox

Configure sandbox using adapter factories when you call `agentStart`. The adapter pattern provides a clean, type-safe interface for different sandbox providers. Without this option, the runtime will not bind `sandbox` in context and OS tools will be unavailable.
...
<Cards>
  <Card title="Secondary Memory" href="/docs/concepts/secondary-memory" />
