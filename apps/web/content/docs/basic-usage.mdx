...
const agent = new Agent({
  ...
  tools: {
    ...agentTools, // task tools are safe without sandbox
...
- Built-in task tools (`todoRead`, `todoWrite`): available via `agentTools`
- OS tools (`bash`, `read`, `write`, etc.): add `osTools` only when you configured `sandbox`; otherwise they will return “Sandbox not configured”
- Custom tools: add your own utilities to `tools`
...
export default function ThreadPage() {
  ...
  return (
    <div>
      ...
      {status === "streaming" && <span>Receiving response…</span>}
      <input
        onKeyDown={(e) => {
          if (e.key === "Enter") {
